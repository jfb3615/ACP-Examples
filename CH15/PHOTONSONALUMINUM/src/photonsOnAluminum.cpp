...
int main (int argc, char * * argv) {
  ...
  for (size_t p=1;p<data.size();p++) {
    ...
    PlotFunction1D *pCubicSpline=new PlotFunction1D(exp(cubicSpline(log)));
    {
  ...
  for (size_t v=0;v<viewSet.size();v++) {
    ...
    statStream << PlotStream::Clear()
	       ...
	       << PlotStream::Size(12)
	       << PlotStream::Color(colors[0])
	       << "― Rayleigh Scattering\n"
	       << PlotStream::Color(colors[1])
	       << "― Compton Scattering\n"
	       << PlotStream::Color(colors[2])
