...
// GLOBALS
// a global pointer for a shared node
SoGroup* branch = 0;
...
void buildBasicLine()
{
	// Construct parts for single basic branch
	SoCoordinate3* points = new SoCoordinate3;
	// Setting two points: the start and the end of the line
	const SbVec3f p1 =  SbVec3f(0, 0, 0);
	const SbVec3f p2 =  SbVec3f(0, 1, 0);
	...
	// Put branch parts together
	branch = new SoGroup;
	branch->addChild(points);
	branch->addChild(pLineSet);
}

//___________________________________________________________________
void drawLine(float x, float y, float len, SoGroup* group)
{
	// Declare a new Separator to contain the line
	SoSeparator *sep = new SoSeparator;


	// A transformation for the main branch
	SoTransform *mainBranchTransform = new SoTransform;
	// translate the main branch to (x,y,0)
	mainBranchTransform->translation = SbVec3f(x, y, 0);
	// scale the branch line over the y axis, to match 'len' length
	mainBranchTransform->scaleFactor.setValue(1, len, 1);

	// Draw the "mother branch" line in the current separator
	sep->addChild(mainBranchTransform);
	sep->addChild(branch); // here we use a shared node
...
{           
	if (nGenMax > 0) {  // Stop the recursion if all generations have been
		...
		// create the 'mother branch'
		drawLine(x, y, len, group);
...
int main(int argc, char ** argv)
{
	...
	// build the basic module of our visualization
	buildBasicLine();
	...
	// and the base color (in RGB mode)
	SoBaseColor *color = new SoBaseColor;
	color->rgb = SbColor(0, 0, 0); // Black
	root->addChild(color);
