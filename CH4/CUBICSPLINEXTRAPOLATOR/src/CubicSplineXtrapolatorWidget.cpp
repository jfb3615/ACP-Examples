...
class CubicSplineXtrapolatorWidget::Clockwork {
...
CubicSplineXtrapolatorWidget::CubicSplineXtrapolatorWidget(QWidget *parent, const std::string  filename):QMainWindow(parent),c(new Clockwork)
{
  c->currentItem=NULL;
  c->counter=0;
  ui.setupUi(this);
...
void CubicSplineXtrapolatorWidget::add() {
...
void CubicSplineXtrapolatorWidget::del() {
  if (c->currentItem) {
...
void CubicSplineXtrapolatorWidget::quit() {
  exit(0);
}

void CubicSplineXtrapolatorWidget::functionSelect() {
 ...
 CubicSplineXtrapolatorWidget::~CubicSplineXtrapolatorWidget() {
...
void CubicSplineXtrapolatorWidget::point(double x, double y) {
...
void CubicSplineXtrapolatorWidget::refresh() {
  ...
  if (ui.actionInterpolate->isChecked()||ui.actionExtrapolate->isChecked()) {
    ...
    while (cx!=end) {
      ...
      Genfun::CubicSplinePolynomial IP;
