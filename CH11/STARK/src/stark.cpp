...
int main (int argc, char * * argv) {
  ...
  const std::string usage=std::string(argv[0]) + " [E=val] [Theta=val] [Phi=val] [TimeUnit=val] [pS=val]";
  //
  // Parse the input ------------------------------------------------------
  //
  NumericInput input;
  input.declare("E", "Electric Field", 0.1);
  ...
  Genfun::GENFUNCTION H = 
    0.5*(p[0]*p[0]+p[1]*p[1]+p[2]*p[2])-1/Genfun::Sqrt()(r[0]*r[0]+r[1]*r[1]+r[2]*r[2])-
    E*sin(theta)*cos(phi)*r[0]-E*sin(theta)*sin(phi)*r[1]-E*cos(theta)*r[2];
  //
